...
namespace MageBackend.Features.AuditExplorer
{
    ...
    public class AuditExplorerController : ControllerBase
    {
        ...
        public AuditExplorerController(ApplicationDbContext context)
        {
            _context = context;
        }
        ...
        public async Task<IActionResult> GetAuditLogs([FromQuery] int page = 0, [FromQuery] int size = 15, [FromQuery] string search = "")
        {
            ...
            if (!string.IsNullOrEmpty(search))
            {
                query = query.Where(a =>
                    ...
                    (a.TableName != null && a.TableName.Contains(search)) ||
                    (a.ActionType != null && a.ActionType.Contains(search)) ||
                    (a.BaseUrl != null && a.BaseUrl.Contains(search))
                );
            }
        ...
        public async Task<IActionResult> GetErrorLogs([FromQuery] int page = 0, [FromQuery] int size = 15, [FromQuery] string search = "")
        {
            ...
            if (!string.IsNullOrEmpty(search))
            {
                query = query.Where(e =>
                    ...
                    (e.ErrorMessage != null && e.ErrorMessage.Contains(search)) ||
                    (e.ErrorData != null && e.ErrorData.Contains(search))
                );
            }
    ...
    public static class AuditExplorerView
    {
